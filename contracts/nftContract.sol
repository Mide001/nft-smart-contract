...
contract nftContract is ERC721Enumerable, Ownable {
    ...
    mapping(address => bool) public isOphirListed;
    mapping(address => bool) public hasRedeemed;
    uint256 public freeNFTAlreadyMinted = 0;
    uint256 public  NUM_FREE_MINTS = 100;
    bool public OHpaused = false;
    string public hiddenURI;


    constructor(string memory _name, string memory _symbol, address payable _treasury) ERC721(_name, _symbol) {
    ...
    function ophirListedMint() external {
        uint256 supply = totalSupply();

        require(!OHpaused, "Ophir Certificate minting is over!");
        require(isOphirListed[msg.sender],  "You are not OphirListed");
    ...
    function setBaseExtension(string memory _newBaseExtension) public authorized {
    ...
    function setOHPaused() external authorized {
    OHpaused = !OHpaused;
    }
    ...
    function addToOphirListed(address[] memory _users) public authorized {
		require(_users.length > 0,"No Users");
        for (uint256 i = 0; i < _users.length; i++) {
            isOphirListed[_users[i]] = true;
        }
    }

	function removeFromOphirListed(address[] memory _user) public authorized {
		require(_user.length > 0,"No User");
		for(uint256 i = 0;i < _user.length;i++)
        isOphirListed[_user[i]] = false;
    }
